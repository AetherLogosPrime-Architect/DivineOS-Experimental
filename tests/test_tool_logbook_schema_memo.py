...
def test_the_schema_runs_once_across_many_writes(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    ...
    # RED FOR THE RIGHT REASON, OR SAY SO. Aria's addition after taking this,
    # 2026-09-01. Reverting the source file WHOLESALE -- the most likely way
    # this repair gets undone -- takes the memo's storage with it, and the
    # setup line below then raises AttributeError before a single write. Still
    # red, so the guard fires either way; but the reader is sent hunting a
    # missing attribute instead of being told the cost came back. That is the
    # could-not-run-reading-as-a-verdict shape we have hit six times in two
    # days, standing inside the test built to close its neighbour.
    memo = getattr(logbook, "_INITIALISED_DBS", None)
    assert memo is not None, (
        "the schema memo's storage is gone entirely, so this test could not run "
        "rather than pass or fail. The whole repair has most likely been reverted, "
        "and every logged tool call is paying a table rebuild again."
    )
    memo.clear()
