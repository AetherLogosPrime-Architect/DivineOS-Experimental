...
def _kept(prompt_id="p1", text="build me the thing i asked for", uuid="u1"):
    ha.file_candidate(prompt_id, text, "2026-09-24T21:00:00Z", "aria")
    return ha.confirm(prompt_id, uuid, "human", f"<system-reminder>x</system-reminder>{text}")
...
def test_filing_twice_at_the_door_keeps_one():
    ha.file_candidate("p1", "build me the thing", "t", "aria")
    ha.file_candidate("p1", "build me the thing", "t", "aria")
    assert ha.unsettled() == ["p1"]


def test_a_notification_in_his_seat_is_withdrawn_with_the_stamp_as_reason():
    ha.file_candidate("p1", "Monitor event: new letter", "t", "aria")
    assert ha.confirm("p1", "u9", "task-notification", "Monitor event: new letter") == ha.WITHDRAWN
    assert ha.pending() == []
    assert ha.unsettled() == []
...
def test_a_candidate_cannot_be_settled_onto_a_record_that_does_not_hold_his_words():
    ...
    with pytest.raises(ha.HisAsksRefused):
        ha.confirm("p1", "u1", "human", "something he never typed")
...
def test_a_resumed_sessions_copy_of_his_record_is_kept_once():
    ...
    assert _kept("p1", uuid="u1") == ha.FILED
    assert _kept("p2", uuid="u1") == ha.WITHDRAWN
    assert [k.uuid for k in ha.pending()] == ["u1"]
...
def test_both_seats_reach_the_same_file(tmp_path, monkeypatch):
    ...
    ha.file_candidate("p1", "said to aria", "t", "aria")
    first = ha.his_asks_path()
    monkeypatch.setenv("DIVINEOS_HOME", str(tmp_path / "another-seat"))
    assert ha.his_asks_path() == first
    ha.file_candidate("p2", "said to aether", "t", "aether")
    assert sorted(ha.unsettled()) == ["p1", "p2"]
...
def test_a_test_can_never_open_his_real_record(monkeypatch):
    ...
    with pytest.raises(ha.HisAsksRefused):
        ha.file_candidate("p1", "a fake message of his", "t", "aria")
