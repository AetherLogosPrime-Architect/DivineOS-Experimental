...
def test_a_declared_reason_comes_back_as_its_own_words(hook):
    ...
    assert "INTENTIONALLY UNWIRED" in got
    assert "opt-in" in got, "the reason itself travels, not a flag"


def test_superseded_counts_as_a_reason(hook):
    p = hook("#!/bin/bash\n# SUPERSEDED by the router; logic moved.\n")
    assert "SUPERSEDED" in (declared_off_reason(p) or "")
...
def test_a_hook_declaring_nothing_returns_none(hook):
    ...
    assert declared_off_reason(p) is None


def test_an_unreadable_file_does_not_invent_a_reason(tmp_path):
    assert declared_off_reason(tmp_path / "nope.sh") is None
...
def test_every_currently_dark_hook_in_the_tree_declares_something():
    ...
    silent = [
        r["name"]
        for r in collect()
        if not r["wired"] and declared_off_reason(HOOKS_DIR / r["name"]) is None
    ]
