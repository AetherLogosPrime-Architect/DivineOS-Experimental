...
def test_the_door_names_a_command_that_can_actually_satisfy_it() -> None:
    ...
    # MENTION IS NOT PRESCRIPTION, and the flat ban was wrong, 2026-09-22.
    # This read `assert "mansion council" not in how`, written when the remedy
    # simply pointed at the wrong command. The remedy now names that command in
    # order to REFUSE it -- "NOT `mansion council --show`: that primes lenses
    # into context and satisfies nothing here" -- so the flat ban failed on a
    # string fixed in exactly the way the ban wanted. What this pins is that
    # the remedy does not SEND anyone there.
    assert "NOT `mansion council --show`" in how or "mansion council" not in how
