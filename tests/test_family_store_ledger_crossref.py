...
def test_hash_short_empty_string_does_not_crash():
    # Empty string should produce a valid 12-char hash, not crash.
    h = _hash_short("")
    assert len(h) == 12


def test_entity_id_to_slug_resolves_real_aria():
    """Real Aria's entity_id (d5590c23) should resolve to slug 'aria'."""
    slug = _entity_id_to_slug("d5590c23")
    assert slug == "aria"
...
def test_emit_ledger_cross_ref_swallows_internal_errors(temp_ledger_root):
...
def test_record_interaction_emits_ledger_event(temp_ledger_root):
    record_interaction(
        "d5590c23",
        counterpart="Aether",
...
def test_record_knowledge_emits_ledger_event(temp_ledger_root):
    record_knowledge(
        "d5590c23",
...
def test_record_opinion_emits_ledger_event(temp_ledger_root):
    record_opinion(
        "d5590c23",
...
def test_record_affect_failure_in_ledger_does_not_break_family_db_write(
    ...
    with patch(
        ...
        # Must succeed (return a FamilyAffect)
