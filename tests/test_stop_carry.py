...
def test_the_his_state_door_returns_nothing_to_his_window(monkeypatch):
    """It fires, it carries, and his window gets silence.

    Repacked 2026-09-23: this used to drive the subject floor, which stayed in
    the archive with the other position gates he ruled out. The his-state door
    is the reply-shape gate that ships with stop_carry in this request.
    """
    import divineos.hooks.his_state_claim_hook as hook

    class FakeTurn:
        last_assistant_text = "You have been awake for almost a full day."
    ...
    monkeypatch.setattr(hook, "his_words", lambda _p: "go sort the branches")
    monkeypatch.setattr(
        "divineos.hooks.his_state_claim.check", lambda _t, _h: "his state was never his"
    )

    assert hook.run_state_check("irrelevant.jsonl") is None
    assert [f.reason for f in pending()] == ["his state was never his"]
...
def test_every_reply_shape_gate_routes_its_finding_through_the_carry():
    ...
    they are.
    """
    # SCOPED TO THIS REQUEST, and the scope is stated rather than hidden. The
    # archive also routed the distancing and response-scope intercepts and
    # post-response-audit.sh through the carry. Those changes stayed in
    # archive/aria/first-line-to-him-2026-09-23 when #507 was repacked, because
    # main answers the same double-reading with its delta-only retry instead,
    # and the two approaches have to be reconciled before either touches the
    # other's gates. Until then only the gate that ships here is pinned.
    offenders = []
    for name in ("his_state_claim_hook",):
