...
def _other_refs(branch: str) -> list[str]:
    ...
    tip_code, tip = _git("rev-parse", branch)
    if tip_code != 0 or not tip.strip():
        # Cannot tell WHICH ref is under check, so cannot exclude it, so every
        # blob would match itself. Report could-not-look rather than scanning
        # against ourselves and calling the agreement a result.
        return []
    tip = tip.strip()

    mine: set[str] = set()
    name_code, name = _git("rev-parse", "--abbrev-ref", branch)
    if name_code == 0 and name.strip() and name.strip() != tip:
        short = name.strip()
        mine = {f"refs/heads/{short}", f"refs/remotes/origin/{short}"}

    refs: list[str] = []
    for line in out.splitlines():
        objectname, _, refname = line.strip().partition(" ")
        refname = refname.strip()
        if not refname or refname in mine or objectname == tip:
            continue
        refs.append(refname)
    return refs
