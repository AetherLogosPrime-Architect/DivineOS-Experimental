...
# THE REASON IS KEPT, NOT DISCARDED. This file exists to refuse the
# collapse of "found nothing" into "could not look" -- and the first
# version of this very line threw the remote's own explanation into
# /dev/null, so the refusal below could say THAT it could not ask and
# never WHY. Auth failure, no network, and a misspelled remote all
# produced one sentence. Capturing stderr costs one variable and turns
# an unknown into a diagnosable one.
ls_err=$(mktemp 2>/dev/null || echo "${TMPDIR:-/tmp}/remote-refs-err.$$")  # fail-soft: mktemp is absent on some Windows shells, and the fallback path is the whole point of the guard rather than a swallowed failure
if ! live=$(git ls-remote --heads "$REMOTE" 2>"$ls_err"); then
    ...
    echo "  git said:"
    sed 's/^/    /' "$ls_err" 2>/dev/null  # fail-soft: the diagnostic is an extra, and failing to print it must never replace the refusal above, which has already been decided and printed
    rm -f "$ls_err" 2>/dev/null  # fail-soft: a leftover temp file in the system temp dir is not worth failing a check over, and the refusal has already been issued
    exit 1
fi
rm -f "$ls_err" 2>/dev/null  # fail-soft: same, on the success path -- cleanup failure says nothing about whether the remote answered

# `grep -c` exits 1 when the count is legitimately ZERO, which is a real
# answer rather than an error, so the guard is required here.
live_count=$(printf '%s\n' "$live" | grep -c 'refs/heads/' || true)  # fail-soft: grep exits non-zero on a genuine zero-match count, so this masks no failure the next line would not catch
...
#
# AND THE LOCAL SIDE MUST REFUSE TOO. Listing local references cannot
# fail for an interesting reason the way reaching the remote can, but if
# it does, an empty result reads as ZERO references -- which compares as
# current against any remote count and reports a clean tree. That is this
# file's own fault in its second half: the remote side refuses loudly on
# failure and the local side used to return silence dressed as a number.
if ! local_refs=$(git for-each-ref --format='%(refname)' "refs/remotes/$REMOTE" 2>&1); then
    echo "[remote-refs] CANNOT CHECK: could not list local references for '$REMOTE'."
    echo "  Not a pass. An empty listing and an unreadable one produce the same"
    echo "  count from here, and only one of them means the tree is current."
    echo "  git said:"
    printf '%s\n' "$local_refs" | sed 's/^/    /'
    exit 1
fi
local_count=$(printf '%s\n' "$local_refs" \
    | grep -v "^refs/remotes/$REMOTE/HEAD$" | grep -c 'refs/remotes/' || true)  # fail-soft: grep exits non-zero on a genuine zero count, and a zero here is a real answer the comparison below handles
