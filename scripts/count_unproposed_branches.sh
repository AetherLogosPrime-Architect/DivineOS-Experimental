...
PR_HEADS="$(gh pr list --state all --limit 400 --json headRefName --jq '.[].headRefName' 2>/dev/null | sort -u)"
if [ -z "$PR_HEADS" ]; then
...
LIVE="$(git branch -r 2>/dev/null | sed -n 's|^[[:space:]]*origin/||p' | grep -v '^HEAD\|^main$' | sort -u)"
[ -n "$LIVE" ] || exit 0
...
if [ "${1:-}" = "--list" ]; then
    while IFS= read -r branch; do
        ...
        ahead="$(git rev-list --count "origin/main..origin/$branch" 2>/dev/null || echo 0)"
        [ "$ahead" = "0" ] && continue
        last="$(git log -1 --format=%cs "origin/$branch" 2>/dev/null || echo unknown)"
