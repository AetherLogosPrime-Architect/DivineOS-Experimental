...
class Branch:
    ...
    substrate_files: frozenset[str] = frozenset()
        ...
        out.append(
            Branch(
                ...
                substrate_files=frozenset(
                    f for f in files if f and f.startswith(_SUBSTRATE_PREFIXES)
                ),
            )
        )
    return out
...
def sweep_signatures(branches: list[Branch], name: str, limit: int = 8) -> list[tuple[str, int]]:
    """Branches sharing WRITING with this one but no code at all.

    Aria's amendment, 2026-09-15, and she won the argument with a sentence from
    my own letter: I had excluded writing files from the comparison entirely,
    and my own account of how the candidate list got built was that the fix was
    to stop the existing instrument DISCARDING what it already had. Excluding
    them from the ranking is right -- two branches sharing four hundred swept
    letters are not solving one problem, and letting that float them to the top
    would drown the signal the door exists for. Throwing the count away is a
    different act, and it is the one I did without noticing.

    So the count is kept and reported separately, under a name that says which
    question it answers. A pair of branches with identical swept letters and no
    code in common is not a duplicate-work signal. It is the CHECKPOINT having
    been in both places -- a defect we spent a whole session paying for, and a
    fact worth putting in front of whoever is looking.

    Wrong answer to the question being asked is not the same as no information.
    """
    by_name = {b.name: b for b in branches}
    mine = by_name.get(name)
    if mine is None or not mine.substrate_files:
        return []
    scored = [
        (b.name, len(mine.substrate_files & b.substrate_files))
        for b in branches
        if b.name != name
        and not (mine.code_files & b.code_files)
        and (mine.substrate_files & b.substrate_files)
    ]
...
def main() -> int:
    ...
    if len(sys.argv) > 2 and sys.argv[1] == "--neighbours":
        ...
        sweeps = sweep_signatures(branches, target)
        if sweeps:
            print("\n# No shared code, but writing in common -- a SWEEP SIGNATURE\n")
            for other, shared in sweeps:
                print(f"  {other}    {shared} writing file(s) in common")
            print("\n  This is not a relationship between the branches. It is the")
            print("  checkpoint having been in both places. Kept because it is")
            print("  real information about a real defect, and reported apart")
            print("  because it is the wrong answer to the duplicate question.")
        return 0
