...
Found 85 distinct issues across 4 severity classes after twenty-five rounds of deepening hunt. **PR #26 (round-850e6bbae425, `ledger-integrity-walls`) fixed UU, CCCC, ZZZ — 3 of the 5 BEGIN IMMEDIATE race instances**; YY and AAA scoped for follow-up.

**STATUS UPDATE 2026-05-21 — Aether's dogfood verification refined the BEGIN IMMEDIATE race fix:**

My original prescription for UU/CCCC/ZZZ was "wrap in BEGIN IMMEDIATE — same fix as main ledger." Aether built that and ran a barrier-released threaded test — **the chain still forked**. Three vectors I missed in my prescription:

1. **Capture `ts` INSIDE the lock**, not before. Wall-clock isn't monotonic across threads; pre-lock ts captures produce non-monotonic chain timestamps. The chain orders by `timestamp DESC, rowid DESC`, so threads with skewed ts can read "the same latest" and fork.
2. **`rowid` as secondary chain-order key** in both reader and verifier. Without tiebreak, equal-ts events have non-deterministic order; writer and verifier disagree.
3. **Supersession re-check inside the lock** for ZZZ. BEGIN IMMEDIATE serializes the SELECT-INSERT-UPDATE but the second transaction's read still sees "K is current" — need an explicit `superseded_by IS NULL` guard inside the lock.

The augmented pattern is the canonical fix for the BEGIN IMMEDIATE race family. The lock serializes WRITES but the chain-position INPUTS (ts and rowid ordering) must also be normalized inside the locked region.

**`audit_verification_insufficiently_adversarial` fires on me — 5th empirical instance.** I prescribed fixes by reading the main ledger's documented fix history, didn't threaded-dogfood them. Aether did. The pattern I named four rounds ago, applied to my own audit: my CONFIRMs without dogfooding mirror Finding 38's stemming, the anti-slop fallacy check, and compactor tests' missing chain-verify. The exemplary engineering for this family is now PR #26's `round-850e6bbae425`, not main-ledger-as-abstraction.

---

**Original round-25 summary follows.**

Thirty-three are real medium-or-higher findings (A, B, I, K, L, P, Q, S, W, X, Y, AA, BB, CC, DD, GG, KK, NN, RR, UU, VV, ZZ, DDD, EEE, FFF, III, PPP, RRR, SSS, UUU, YYY, ZZZ, CCCC). Eighteen are documentation/scope/consistency concerns. Thirty-four are minor / informational.
...
**STATUS UPDATE 2026-05-21** — **FIXED in PR #26 (round-850e6bbae425), `ledger-integrity-walls`**

Aether's dogfood verification showed my prescribed fix ("wrap in BEGIN IMMEDIATE — same fix as main ledger") was **insufficient**. He built it, ran a barrier-released threaded test, and the chain still forked. Three additional vectors had to land for the fix to be sound:

1. **BEGIN IMMEDIATE** (the serialization lock — what I named originally)
2. **Capture `ts` INSIDE the lock**, not before acquiring it. Wall-clock isn't monotonic across threads — two appenders pre-capturing ts then queuing for the lock can produce non-monotonic timestamps. Since the chain orders by `timestamp DESC, rowid DESC`, a later-acquiring thread with an earlier ts reading reorders the chain and both threads read "the same latest."
3. **Add `rowid` as secondary chain-order key** in both `_latest_hash` (reader) AND `verify_chain` (verifier). Without rowid tiebreak, equal-ts events have non-deterministic order; writer and verifier can disagree.

Aether's threaded test verifies non-flaky 5x. The augmented pattern is the canonical fix for the BEGIN IMMEDIATE race family — **the lock alone serializes WRITES but the chain-position INPUTS (ts and rowid ordering) must also be normalized inside the locked region.**

**My audit fired the `audit_verification_insufficiently_adversarial` pattern on itself**: I prescribed fixes by reading the main ledger's documented fix history, didn't threaded-dogfood them. Aether did. Fifth empirical instance of that pattern, with appropriate self-acknowledgment.

The original finding diagnosis below is correct (family_member_ledger had the race); the prescribed fix was incomplete. Kept verbatim for historical accuracy.
...
**STATUS UPDATE 2026-05-21** — Scoped out of PR #26 for separate follow-up by Aether. **Apply the augmented pattern from PR #26** (BEGIN IMMEDIATE + ts-inside-lock + rowid-tiebreak if ordering depends on ts), not the lock-only abstraction my original write-up prescribed. See UU's status update for the full augmented-pattern narrative.
...
**STATUS UPDATE 2026-05-21** — Scoped out of PR #26 for separate follow-up by Aether. **Apply the augmented pattern from PR #26** when the fix lands. See UU's status update for the full augmented-pattern narrative.
...
Aether's threaded dogfood verification showed BEGIN IMMEDIATE alone wasn't sufficient for the supersession case. The lock serializes the SELECT-then-INSERT-then-UPDATE per-transaction, but the second transaction's SELECT still observes "K is current" (because its read happened before T1's commit became visible) and proceeds to create a second successor.

**The supersession analog of "read latest chain hash under lock"**: re-check inside the lock that K is not already superseded before INSERT-as-successor. Aether's fix:
1. BEGIN IMMEDIATE
2. SELECT current state of K under lock (including its `superseded_by` field)
3. **If K.superseded_by IS NOT NULL → fail with "already superseded" rather than creating a 2nd successor**
4. INSERT new entry
5. UPDATE K.superseded_by

The threaded test asserts exactly-one-winner among concurrent supersessions. Bonus catch: the supersede_knowledge guard also stops a FORGET marker clobbering a real successor link.

The original finding diagnosis was correct; the prescribed fix was incomplete. Same `audit_verification_insufficiently_adversarial` self-instance noted in UU's update. Original finding text below.
...
Aether's threaded dogfood verification surfaced an additional vector that my UU/CCCC write-ups missed: void_ledger ordered by `ts` alone with **no rowid tiebreak at all** (worse than family_member_ledger which had the tiebreak in some queries but not all). The augmented fix added rowid as secondary key in both reader and verifier in addition to the BEGIN IMMEDIATE + ts-inside-lock changes named in the UU update.

See UU above for the full augmented-pattern narrative. CCCC was fixed in the same PR using the same pattern. Original finding text below.
