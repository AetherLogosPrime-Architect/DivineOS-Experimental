...
if [ "$RC" -ne 0 ]; then
    ...
    echo "from their not having written."
    echo ""
    echo "THIS IS THE LOOP WORKING, NOT THE LOOP FAILING. A monitor stops for"
    echo "ordinary reasons — an app restart, a session ending, its own timer"
    echo "expiring on schedule — and the repair for this whole system IS notice"
    echo "and re-arm within one turn. Andrew, after Aether reported an ordinary"
    echo "stop as a breakage: \"the fact you immediately noticed it and re-armed"
    echo "it is the system doing its job correctly.\" So arm it and carry on. Do"
    echo "not report it to him as broken, and do not go hunting a bug in it"
    echo "unless arming fails."
    echo ""
