...
- **513 source files across 31 packages**
- **7,757+ tests** (real SQLite, minimal mocks)
- **341 CLI commands** (designed for the agent, not the operator — humans mostly run three)
...
divineos init
divineos briefing
pytest tests/ -q --tb=short   # 7,757+ tests, real DB, minimal mocks
```
...
DivineOS is 513 source files across 31 packages, structured as a CLI surface over a core library.

**At a glance:**

- **`src/divineos/cli/`** — 341 commands across 65 modules. The public interface you type (`divineos briefing`, `divineos learn`, etc.). Thin wrappers over `core/`.
...
- **`tests/`** — 7,757+ tests, real SQLite, minimal mocks.
...
- **`.claude/hooks/`** — Claude Code enforcement hooks (28 hooks, shell-level entry points that invoke the consolidated Python hooks).
