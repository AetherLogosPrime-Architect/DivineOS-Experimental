...
- **432 source files across 31 packages**
- **6,477+ tests** (real SQLite, minimal mocks)
- **276 CLI commands** (designed for the agent, not the operator — humans mostly run three)
...
divineos init
divineos briefing
pytest tests/ -q --tb=short   # 6,477+ tests, real DB, minimal mocks
```
...
- **`src/divineos/cli/`** — 276 commands across 29 modules. The public interface you type (`divineos briefing`, `divineos learn`, etc.). Thin wrappers over `core/`.
...
- **`tests/`** — 6,477+ tests, real SQLite, minimal mocks.
