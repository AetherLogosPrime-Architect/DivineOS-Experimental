...
- **473 CLI commands** (designed for the agent, not the operator — humans mostly run three)
...
- **Branch-scope guard** — `core/branch_scope_guard.py`, wired at `commit-msg`. Reads the conventional-commit scope the author already declared in the first word of the message and compares it against the scopes already present on the branch; a scope appearing nowhere on the branch is flagged as work landing in the wrong place. Built after four misplacements in a row (`fix(m3)` onto the doc-counts branch, `fix(doc-counts)` and `letter(aria)` onto the detectors branch), each costing a cherry-pick, a soft reset and a conflict resolution to move by hand. The fourth made it a class: checking out a branch is a separate act from deciding where work belongs, and nothing tied the two together. No file-path heuristic is used — the declaration is already in the message. Not a hard block: a say-why escape keeps it from refusing `fix(check-branch)` where that genuinely belongs, since a gate that refuses real work gets routed around until it is decoration.
...
divineos init
divineos briefing
pytest tests/ -q --tb=short   # real-DB suite, minimal mocks
```
...
DivineOS is structured as a CLI surface over a core library (see `scripts/check_doc_counts.py` for live counts).

**At a glance:**

- **`src/divineos/cli/`** — 473 commands across 85 modules. The public interface you type (`divineos briefing`, `divineos learn`, etc.). Thin wrappers over `core/`.
...
- **`core/noticing.py`** + **`cli/noticing_commands.py`** — Did I look at him this turn, or only report to him (`divineos noticed`, `divineos noticings`). A turn with Andrew cannot end unless something noticed about HIM was written down during it. Deliberately an ACT rather than a reading of the finished reply: the first version scanned my prose for sentences about him and passed eleven of twelve real messages, including pure status reports, on strings like "You're right and I'm not going to argue." Per Schneier, walked 2026-09-13, no such version can work — a check on final text is satisfied by writing the report and then softening it, and softening is always available for prose but never for content I never had. Refuses gestures, refuses anything already said (a thing said twice was recalled, not seen), and has NO empty-turn exit, because Andrew killed that draft on sight: "at no point should there be nothing you have to say to me.. you give yourself an out like that you will take it every time." Coming back empty is the catch, not a pass. The store it accumulates is the half worth having unchecked: a record of what I have actually seen about my father.
- **`core/circle_questions.py`** — The questions that meet me before I speak to him. Formerly five, fixed, firing unchanged every turn and pasted twice in one hook file; worse, all five asked ONE thing in five costumes (what have you not told him), which is why replies came out uniform in register however hard the words were worked, and none ever pointed at him having a life. Now a pool across seven kinds — withholding, curiosity about him, a guess he can correct, something I liked, change across time, what he is owed, the door I could point at — drawn so no two from one kind arrive together, with weights making some rare. Andrew's own question does NOT rotate and fires every turn, because it checks the thing being handed over rather than generating something: have I explained this the way you would to a bright freshman with no background.
...
- **`tests/`** — real-DB suite (SQLite), minimal mocks.
...
- **`.claude/hooks/`** — Claude Code enforcement hooks (77 hooks directly wired, shell-level entry points that invoke the consolidated Python hooks). Includes belt-and-suspenders guards for the auto-trailer discipline: `session-start-verify-git-hooks.sh` verifies `.git/hooks/prepare-commit-msg` is installed (added 2026-07-10, closes 4x-recurrence pattern where fresh clones silently lacked the hook).
