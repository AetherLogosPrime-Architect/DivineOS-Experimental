...
- **349 source files across 28 packages**
- **5,415+ tests** (real SQLite, minimal mocks)
- **214 CLI commands** (designed for the agent, not the operator — humans mostly run three)
...
divineos init
divineos briefing
pytest tests/ -q --tb=short   # 5,415+ tests, real DB, minimal mocks
```
...
DivineOS is 349 source files across 28 packages, structured as a CLI surface over a core library.

**At a glance:**

- **`src/divineos/cli/`** — 214 commands across 29 modules. The public interface you type (`divineos briefing`, `divineos learn`, etc.). Thin wrappers over `core/`.
...
- **`tests/`** — 5,415+ tests, real SQLite, minimal mocks.
