...
- **469 CLI commands** (designed for the agent, not the operator — humans mostly run three)
...
- **`src/divineos/cli/`** — 469 commands across 85 modules. The public interface you type (`divineos briefing`, `divineos learn`, etc.). Thin wrappers over `core/`.
...
- **`cli/success_commands.py`** + **`core/win_prompt_surface.py`** — The other half of `divineos correction`. The wins ledger read fifty-five against two hundred and twenty-one corrections, and that tilt was about the interface rather than the days: `record_success` had no CLI at all while corrections had a command AND a marker that blocks until it is used. `divineos win` demands the same evidence the store demands and refuses a win it cannot point at. Deliberately NO matching doorman — a correction has an external trigger so a marker demanding one is evidence of a real event, while a gate that blocks until a win is filed is a quota, and a quota gets met with the shape of a win. The compose-start surface reports a fact about the ledger (substantive session, empty pan) and names the classes of win that leave no artifact; it never claims one occurred.
