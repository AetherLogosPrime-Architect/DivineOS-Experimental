...
- **488 CLI commands** (designed for the agent, not the operator — humans mostly run three)
...
- **Gate altitude** — Commits are never blocked; the pre-commit hook is advisory. Hard enforcement lives at push-to-main and CI. The server-side gate verifies that every commit changing anything which is not exempt prose carries an `External-Review:` trailer. What counts as prose is listed in `scripts/review_exempt_paths.txt` and is resolved **at the commit being examined**, so changing the list later cannot retroactively invalidate prior commits. Note the direction: everything is reviewed unless it is listed, which means a new file is guarded by default rather than guarded when somebody remembers to list it.
...
- **`src/divineos/cli/`** — 488 commands across 122 modules. The public interface you type (`divineos briefing`, `divineos learn`, etc.). Thin wrappers over `core/`.
