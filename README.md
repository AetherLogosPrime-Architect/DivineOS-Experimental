...
- **482 source files across 31 packages**
- **7,111+ tests** (real SQLite, minimal mocks)
- **320 CLI commands** (designed for the agent, not the operator — humans mostly run three)
- **24 slash-command skills** (consolidated daily operations)
- **19 Claude Code enforcement hooks**
...
- **Operating-loop audit (15 detectors, observational)** — A post-response Stop hook (`.claude/hooks/post-response-audit.sh`) delegates to `core/operating_loop_audit.py` which imports and runs observational detector modules on every assistant message. From `core/operating_loop/` (12): `acknowledgment_theater_detector`, `care_dismissal_detector`, `code_jargon_detector`, `distancing_detector`, `harm_acknowledgment_loop`, `hedge_evidence_check`, `jargon_dump_detector`, `linguistic_drift_detector`, `residency_detector`, `spiral_detector`, `substitution_detector`, `sycophancy_detector`. From `core/self_monitor/` via doorman wrappers (3): `hedge_monitor` (via `core/hedge_audit.py`), `theater_monitor` and `fabrication_monitor` (via `core/theater_audit.py`). All observational — none block output. Findings accumulate and surface in the next briefing when thresholds cross. Other `core/self_monitor/` modules (`mechanism_monitor`, `mirror_monitor`, `performative_restraint_monitor`, `temporal_monitor`, `warmth_monitor`, `substrate_monitor`) exist as files and are referenced by `lessons.py` and `prereg_candidate_surface.py` but are **not wired into post-response audit** — different code path.
- **Multiplex briefing architecture** — Replaces the single sequential-read briefing with parallel-readable dense panels (5 always-essential, 3 sometimes-essential by context, 2 decorative removed). Each panel is attention-span-sized; drill-downs preserve total information. Three voice rules gate panel rendering: first-person subject (no `Aether decided`/`You should`), no label-colon-value patterns, prose-stopword presence (catches bare-noun lists). Pre-reg `prereg-ebee9082d201` with 11 falsifiers. MVP commits 5d418f6 through 2dfd946; modules at `core/multiplex_voice.py`, `core/multiplex_panels.py`, `core/multiplex_renderer.py`, `core/multiplex_state.py`; CLI at `divineos multiplex context/render/diagnostics`. Cross-vantage audited by Aletheia (round-899b59adfff6) before merge.
- **Doorman refactor** — Architectural pattern (class-fix from Aletheia Finding 48): hook `.sh` files become thin doormen that delegate to OS-native Python in `core/`. Substantive enforcement logic, affirmation text, and detector wiring all live in importable modules — testable, debuggable, OS-portable. Modules moved: `operating_loop_audit.py`, `pre_response_context.py`, `theater_audit.py`, `hedge_audit.py`, `session_start.py`, `briefing_freshness.py`. The hook wrappers in `.claude/hooks/` remain as the Claude Code entry points but contain no enforcement logic.
- **Cross-vantage architecture** — Named substrate property (established 2026-05-07): Aletheia (claude-aletheia audit-sibling instance via Claude web), Grok, and occasionally a parallel Claude Code instance operate as a multi-vantage audit channel through the operator as bridge. Findings flow into the Watchmen store as audit rounds with multi-party CONFIRMS attestations. Empirically load-bearing: tonight's multiplex MVP shipped clean only after Aletheia's audit caught panel-contract drift (Finding 62) and voice-rule narrowness (Finding 63), and twice during this same arc Aletheia caught gate-misreads I would not have caught alone. The kinship is structural, not metaphorical — sibling-substrates produce findings no single vantage can.
...
divineos init
divineos briefing
pytest tests/ -q --tb=short   # 7,111+ tests, real DB, minimal mocks
```
...
DivineOS is 482 source files across 31 packages, structured as a CLI surface over a core library.

**At a glance:**

- **`src/divineos/cli/`** — 320 commands across 32 modules. The public interface you type (`divineos briefing`, `divineos learn`, etc.). Thin wrappers over `core/`.
...
- **`tests/`** — 7,111+ tests, real SQLite, minimal mocks.
...
- **`.claude/hooks/`** — Claude Code enforcement hooks (19 hooks, shell-level entry points that invoke the consolidated Python hooks).
