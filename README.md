...
- **8,168+ tests** (real SQLite, minimal mocks)
- **355 CLI commands** (designed for the agent, not the operator — humans mostly run three)
...
divineos init
divineos briefing
pytest tests/ -q --tb=short   # 8,168+ tests, real DB, minimal mocks
```
...
- **`src/divineos/cli/`** — 355 commands across 65 modules. The public interface you type (`divineos briefing`, `divineos learn`, etc.). Thin wrappers over `core/`.
...
- **`tests/`** — 8,168+ tests, real SQLite, minimal mocks.
