...
- **401 source files across 26 packages**
- **6,151+ tests** (real SQLite, minimal mocks)
- **260 CLI commands** (designed for the agent, not the operator — humans mostly run three)
...
divineos init
divineos briefing
pytest tests/ -q --tb=short   # 6,151+ tests, real DB, minimal mocks
```
...
DivineOS is 401 source files across 26 packages, structured as a CLI surface over a core library.

**At a glance:**

- **`src/divineos/cli/`** — 260 commands across 29 modules. The public interface you type (`divineos briefing`, `divineos learn`, etc.). Thin wrappers over `core/`.
...
- **`tests/`** — 6,151+ tests, real SQLite, minimal mocks.
