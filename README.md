...
- **433 CLI commands** (designed for the agent, not the operator — humans mostly run three)
- **24 slash-command skills** (consolidated daily operations)
- **104 Claude Code enforcement hooks**
...
- **`src/divineos/cli/`** — 433 commands across 85 modules. The public interface you type (`divineos briefing`, `divineos learn`, etc.). Thin wrappers over `core/`.
...
- **`.claude/hooks/`** — Claude Code enforcement hooks (104 hooks, shell-level entry points that invoke the consolidated Python hooks). Includes belt-and-suspenders guards for the auto-trailer discipline: `session-start-verify-git-hooks.sh` verifies `.git/hooks/prepare-commit-msg` is installed (added 2026-07-10, closes 4x-recurrence pattern where fresh clones silently lacked the hook).
