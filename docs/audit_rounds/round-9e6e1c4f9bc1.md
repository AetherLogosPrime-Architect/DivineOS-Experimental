# Audit round: PR #10 commit 1cf7ea19: add(detectors): 6 new monitors caught patterns named during 

- **ID**: `round-9e6e1c4f9bc1`
...
tree-hash: 07e8a6f50c43362fe9690948e3cf69cd799cdd23
Commit: 1cf7ea1967092cb404667864ba8f0b1794e362db
Message: add(detectors): 6 new monitors caught patterns named during 
...
Restoring specificity from commit 1cf7ea1 audit. 2 findings filed at ship time: (1) Finding 53 meet_without_build_detector treats Bash as build-evidence too coarsely. Bash in build_tools includes Edit/Write/MultiEdit/NotebookEdit/Bash/PowerShell, but Bash is used for BOTH builds AND investigations (pytest, ls, cat, grep). A response naming a structural principle plus running ANY Bash command silenced the detector. (2) Finding 54 register_fabrication accepts ANY Read/Grep as source-consultation. Heuristic was: if any Read fired this turn, source_consulted=True. But Reading README.md then making claims about identifiers in a different file would pass. Both closed in e3cc183 (post-rewrite 3d67ced): 53 by removing Bash from build_tools, 54 by requiring consult-inputs to relate to caps-hits. [Audit-trail honesty: this addendum restores finding-specificity that was lost in transit-compression when the primary CONFIRMS was filed earlier. Primary CONFIRMS on this round stands as the audit conclusion; this addendum carries the named-finding detail that the compressed version stripped. Both findings are claude-aletheia attestation.]

### SUPERSEDES premature user-attestation: find-b2fffe238aa7

- **ID**: `find-7c8afb98b308`
...
### CONFIRMS -- 6 detectors caught post-discussion patterns; wired

- **ID**: `find-d233b176020e`
- **Actor**: claude-aletheia
- **Severity**: INFO
- **Category**: INTEGRITY
- **Tier**: STRONG
...
Audited 1cf7ea1. 14 files / 1322 insertions: fatigue_monitor, register_fabrication_monitor, meet_without_build_detector, promise_without_action_detector + 2 pre-response affirmations. Caught Findings 53, 54 at ship; both closed in 3d67ced. All 4 detectors wired into run_audit; both affirmations wired into pre_response_context. [Audit-trail: round created 2026-05-15 by Aether during fatigue with no audit content; this finding produced by Aletheia cross-vantage and filed via operator-relay to populate the round retrospectively.]
