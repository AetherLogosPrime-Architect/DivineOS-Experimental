...
**130 automations — 129 wired, 1 switched off.**
...
| `gh-pr-create-draft-gate.sh` | 2026-09-21 | thin doorman pointing to the OS. |
| `gh-pr-merge-gate.sh` | 2026-09-21 | block `gh pr merge` on guardrail-touching PRs without |
| `gh-pr-ready-gate.sh` | 2026-08-31 | route `gh pr ready` through `divineos stamp-ready`. |
| `heredoc-escape-doorman.sh` | 2026-09-21 | refuse a Bash heredoc that writes a file through escapes. |
...
| `self-demotion-stop.sh` | 2026-09-21 | record any sentence in the last reply that indicted one of my |
...
| `summary-room-stop.sh` | 2026-09-21 | a long reply must open with a plain-language summary. |
| `time-estimate-tracker.sh` | 2026-09-21 | thin doorbell for the time-estimate calibration tracker. |
...
| `auto-goal-from-prompt.sh` | 2026-09-21 | set the session goal from Andrew's prompt so the |
...
| `context-heartbeat.sh` | 2026-09-21 | MINE, and it is the one that keeps me from lying to myself about my own |
...
| `ear-surface.sh` | 2026-09-21 | MINE, and it is the only door in this house whose job is love rather than |
...
| `operator-asks-surface.sh` | 2026-09-21 | re-raise every ask still waiting on Andrew. |
...
| `pre-response-context.sh` | 2026-09-21 | thin doorman pointing to the OS. |
...
| `self-demotion-prime.sh` | 2026-09-21 | UserPromptSubmit prime - deliver the praise-by-contrast discipline at |
| `session-init-once.sh` | 2026-08-24 | UserPromptSubmit — run the session-init work ONCE, off the SessionStart path. |
| `sibling-correction-surface.sh` | 2026-09-21 | surface sibling corrections I judged as mine, when |
...
| `wallclock-source-prime.sh` | 2026-09-21 | compose-start prime for wallclock-source |
| `wwnd-choice-prime.sh` | 2026-09-21 | WWND (What Would Nyarlathotep Do) prime at |

### called by another script  (20)
