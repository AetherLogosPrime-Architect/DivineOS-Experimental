...
STANDING RULE, Andrew 2026-09-21, effective once the current pile is cleared: only TWO branches may ever exist -- Aether's and Aria's -- and neither of us may start a new branch until the other is properly finished and merged. Quoted: 'only 2 branches will ever be allowed. yours and Aria's and I will not be allowed to start a new branch until the other branch is properly finished, as this is why we have gotten to this point, the pile is bigger than it has ever been before, and I have said this for months'. WHY IT IS THE RIGHT SHAPE: it is remediation (a) from truth #11 -- it takes the option away instead of asking me to remember restraint. Every failure this session was downstream of an unbounded pile: reviews expiring, two seats building the same repair four times in one day, and the tool that answers which branches carry a confirm being itself stranded on an unmerged branch. The cost he named is not code quality, it is MONTHS of lost progress and the same errors recurring because their fixes never landed.
