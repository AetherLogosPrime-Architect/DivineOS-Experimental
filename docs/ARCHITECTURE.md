...
src/divineos/
  ...
  hooks/                       Hook integration
    ...
    prior_art_hook.py          Runs the prior-art lookup on Grep/Glob, unasked (2026-08-14). NOT a gate: exits 0 on every path, asks nothing, so there is no compliance surface to bypass. core.prior_art was complete and wired to nothing; the missing piece was a trigger, not a tool. Prints only what a working-tree grep structurally cannot see -- other branches, other commits -- which is exactly where two same-day rediscoveries were hiding
