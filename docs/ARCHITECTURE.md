...
src/divineos/
  ...
  cli/                         CLI package (469 commands across 84 modules)
    ...
    success_commands.py        `divineos win` — the other half of `divineos correction`. Demands checkable evidence; deliberately has NO matching doorman, because a gate that blocks until a win is filed is a quota and a quota gets met with the shape of a win.
