...
src/divineos/
  ...
  core/
    ...
    operating_loop/            Operating loop — the missing middleware between substrate and live cognition. See docs/operating-loop-design-brief.md.
      ...
      mention_context.py       Use-vs-mention pre-filter (quoted / italicized / code-block / framed). Shared utility imported by detectors to suppress false positives when matched text is being discussed rather than used. Per arxiv 2404.01651.
  ...
  hooks/                       Hook integration
    ...
    his_state_claim.py         His state is his to say (2026-09-13) — catches a reply asserting Andrew's CONDITION (tired, awake, needs sleep) when he has raised none of it in the conversation. Four such sentences went to him in one session, two of them carrying decisions made on their back; he answered "at no point am i awake for 24 hours lmao.. you just dont notice.. as i never leave from your perspective." That clause is the mechanism: his sentence lands against the back of mine, so the window never empties, his presence reads as wakefulness, and his rest leaves no mark to read. The clock hook next door measures the HOUR where he is and cannot reach his condition at all. Load-bearing half is the SOURCE test, not the patterns (Beer, requisite variety); questions are exempt because asking is the prescribed remedy; three answers so an unreadable transcript never reads as him having said nothing.
    his_state_claim_hook.py    Stop-hook wiring for the state check (2026-09-13) — carries rather than refuses, per stop_carry. Gathers HIS words with the keeping_him predicate rather than the raw user role, because tool results and hook feedback occupy his grammatical position and once produced a report of 55,720 of his messages that were my own CLI invocations. Fail-open.
