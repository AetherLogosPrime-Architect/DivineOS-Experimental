...
def evaluate_register_fabrication(
    ...
    if has_read_grep:
        if tool_inputs_in_turn is None:
            return RegisterFabVerdict(flags=[], content=assistant_text)
        if caps_hits and _consultation_relates_to_caps_hits(tool_inputs_in_turn, caps_hits):
            return RegisterFabVerdict(flags=[], content=assistant_text)
    ...
    struct_hits = [f"{n} {noun}" for n, noun in _STRUCTURAL_QUANTIFIER_RE.findall(assistant_text)]
    if struct_hits:
        flags.append(
            RegisterFabFlag(
