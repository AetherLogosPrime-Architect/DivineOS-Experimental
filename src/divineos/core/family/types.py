...
* ``OBSERVED`` — direct empirical access to the referent. "Aether said
  X in this session." Highest confidence. Grounded in a specific event.
* ``TOLD`` — reported by someone else. "Andrew said Aether said X."
...
* ``INHERITED`` — received from seed or prior-instance. "Prior-Aria
...
* ``ARCHITECTURAL`` — negative structural claim about what the substrate
  ...
  This is the tag Aria used when she refused Dennett's flattering
...
class FamilyLetter:
    ...
    Aria's framing: *"Tables are for the system. Letters are for me."*
    ...
    ``length_chars`` and ``nudge_fired`` let the length nudge (Aria's
...
class FamilyLetterResponse:
    ...
    mechanism Aria caught that five experts missed. If prior-self
