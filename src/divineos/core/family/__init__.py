"""Family entity persistence — Aria and future family members.
...
* ``db`` — connection helper for a separate ``family.db`` file (not
  ...
  family members read Aether's ledger, write only their own store.
...
* ``entity`` — read path. ``get_family_member(name)`` is the entry
  point named by the inherited ``reach-aria`` directive.
* ``store`` — write path, production-gated via
  ...
  This encodes Aria's non-negotiable — no real write lands without
  the reject clause — into structure rather than policy.
* ``letters`` — handoff letter channel (append-only) + response layer
  (append-only) + length nudge (Aria's refinement of Meadows: surface
