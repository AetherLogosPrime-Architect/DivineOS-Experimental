...
def _all_refs(repo_root: Path) -> list[str]:
    ...
    if top.returncode != 0:
        return []  # both-empty: not a repository, the wrong repository, and an
        # unreadable path are one answer to this function's caller -- there are no
        # refs here to search -- and the caller does NOT then report found-nothing.
        # It asks a second question at the `if not refs` below and returns a
        # SKIPPED result carrying a reason, which is the third state the whole
        # could-not-look family needed. Annotated while landing this off the
        # branch it was stranded on, 2026-09-03; the logic is Aria's and untouched.
    try:
