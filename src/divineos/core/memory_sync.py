...
- Auto memories: written by the OS (project state, stats, recent lessons)

Auto memories are things the AI "can't help but remember" — the architecture
forces them to stay current. Manual memories are deliberate choices.

This module writes auto-memory files to the Claude Code memory directory.
It runs during SESSION_END to keep cross-session context accurate.
"""
...
description: Current DivineOS stats and health — auto-updated at SESSION_END, do not edit manually
type: project
---

{chr(10).join("- " + p for p in parts)}

*Auto-generated by DivineOS memory sync. Updated each SESSION_END.*
"""
