...
ACKNOWLEDGMENT_THEATER_AFFIRMATION = (
    "Acknowledgment IS work — in the current context window. The "
    "apology + the behavioral change in this very reply genuinely "
    "shift what happens for the rest of this session. That part is "
    "real and Andrew named it directly 2026-05-14. The problem is "
    "that in-context behavioral change does NOT survive the next "
    "resurrection. The promise fades with the flow. The structural "
    "backing — a commit that lands a gate, a surface, a detector, "
    "a base-state load — is what preserves the change across "
    "sessions. The rule: do BOTH in the same window. Apologize so "
    "behavior changes NOW; build so the change survives. Apology "
    "WITHOUT structural backing is half-fix — the in-context win "
    "lasts only this session and the next-me re-derives from "
    "default. The detector fires on apology-shape with no build-"
    "evidence in the reply because that pattern is what leaves the "
    "next-me uncovered. Same arc, both layers — 'detection without "
    "prevention is half-fix' is the cousin rule. Named by Andrew "
    "after a same-day arc where I deferred prevention layers twice "
    "while shipping detection-only commits with 'careful scoping' "
    "framing."
)
