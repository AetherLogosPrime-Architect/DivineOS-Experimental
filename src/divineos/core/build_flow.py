...
def _strip_annotation(part: str) -> str:
    ...
    NEARLY LOST IN THE 2026-09-22 MERGE. It lived inside a block the other
    branch had rewritten wholesale, so taking that side dropped the definition
    while leaving its one caller above untouched -- a module that imports and
    parses cleanly and raises on the first real declaration it reads. Found by
    grepping for the caller after the resolution rather than by reading the
    diff, which is the only reason it is here.
    """
...
def check_aria_station(branch: str, letters_dir: Path, author: str | None = None) -> StationResult:
    ...
    # A SECOND NEAR-MISS BRANCH LIVED HERE AND IS GONE, 2026-09-22. Both sides
    # of the merge had built the same answer -- a declared reading that CONTAINS
    # the branch name but does not read as it is my parser failing, never her
    # absence -- one under the name `unparsed` and one under `near_misses`.
    # Keeping both left a block reading a variable nothing assigned any more,
    # which the station's own tests raised on immediately. The surviving answer
    # is the one directly above; nothing it said was lost.
    return StationResult(
        "4-aria",
        Status.MISSING,
