...
def _has_solution_shape(reply: str) -> tuple[bool, str | None, str | None]:
    ...
    NOTE 2026-07-25: this lexical detector is being retired per Aria's
    signal-based-gates design (docs/signal-based-gates-design-2026-06-16.md).
    The replacement fires at PreToolUse on substrate-mutating tools and
    reads the action-stream for walk-record / design-doc consultation
    directly, retiring the whole class of language-detection false-fires.
    This function is being kept alive during the migration; new callers
    should route through the signal-based check instead.
    """
