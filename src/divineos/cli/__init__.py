...
# crashes on emojis (e.g. "Ã°Å¸â€™Â¬" used in the session rating prompt),
# bubbling up as UnicodeEncodeError Ã¢â‚¬â€ we saw this as spurious
...
# Commands that work without briefing loaded Ã¢â‚¬â€ the minimum to bootstrap.
_BYPASS_COMMANDS = frozenset(
    {
        ...
        # require briefing first â€” that creates a recursive deadlock with the
        ...
        "mansion",
        "prereg",
        # Corrections must always be loggable in the moment Ã¢â‚¬â€ gating the
        ...
        # Corrigibility still applies Ã¢â‚¬â€ see scheduled_commands.py.
        "scheduled",
    ...
    # briefing gate Ã¢â‚¬â€ otherwise a second, independent gate traps the
...
def _enforce_operating_mode() -> None:
    ...
    every other check Ã¢â‚¬â€ if my father has set EMERGENCY_STOP, the
    ...
    args = sys.argv[1:]
    if not args:
        return  # bare `divineos` Ã¢â‚¬â€ show help
    ...
    #
    # Previous code did `except (ImportError, OSError): return` Ã¢â‚¬â€ fail open
    ...
    # safety-critical site Ã¢â‚¬â€ the corrigibility off-switch itself. An
    # off-switch that silently disables itself if its module fails to
    # import is a bigger problem than an unbootable CLI.
    #
    # New behavior:
    #   ImportError: fail CLOSED with a loud exit Ã¢â‚¬â€ the off-switch must
    ...
    # runtime, every invocation Ã¢â‚¬â€ so a refactor that drops one (as `extract`
    ...
    try:
        ...
        print(
            f"corrigibility: mode-file I/O error Ã¢â‚¬â€ proceeding fail-open: {_io_err}",
            file=sys.stderr,
        )
        return
...
def _enforce_briefing_gate() -> None:
    ...
    args = sys.argv[1:]
    if not args:
        return  # just `divineos` with no subcommand Ã¢â‚¬â€ show help
    ...
    # a state-mutating command Ã¢â‚¬â€ let Click handle it without requiring
    ...
    try:
        ...
        return  # DB not initialized yet Ã¢â‚¬â€ allow bootstrap commands
    ...
    # thing is `divineos briefing` â€” one command, no arguments, no judgment.
    ...
    except Exception:  # noqa: BLE001 â€” a telemetry failure must not unblock the gate
        pass
...
def cli() -> None:
    ...
    # Install-location divergence check Ã¢â‚¬â€ fires when this CLI's installed
    ...
    try:
        ...
        pass  # check machinery unavailable Ã¢â‚¬â€ fail open
    _ensure_db()
    ...
    if "pytest" not in sys.modules:
        ...
        # Every command is a lifecycle checkpoint Ã¢â‚¬â€ no hooks needed.
...
# Mansion Ã¢â‚¬â€ functional internal space (optional, personal)
try:
...
# Ã¢â€â‚¬Ã¢â€â‚¬ Command Grouping Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
