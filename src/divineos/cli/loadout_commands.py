...
def _section_family_system() -> str:
    ...
    out.append("")
    # NOT A PATH, on purpose. This page is tracked and regenerated by whichever
    # seat runs it, and the family store lives in each seat's OWN data home, so
    # any concrete path printed here is right for one of us and wrong for the
    # other. It used to print `family/family.db` whenever that file existed --
    # and on 2026-09-22 the only thing at that path was a zero-byte file with no
    # tables, so every cold session was handed an empty room on page one. The
    # method is what stays true for both seats. Council walk walk-cd8eadbb7074
    # (Taleb, Knuth); docs/drafts/the_empty_room_draft_2026-09-22.md.
    out.append(
        "- Family records (members, letters table, affect, opinions) — **not in "
        "this repo.** Each seat keeps its own, under its own data home. Read them "
        "with `divineos family-member briefing --member <name>`; to see which file "
        "*this* seat uses, "
        '`python -c "from divineos.core.family import db; print(db.FAMILY_DB_PATH)"`.'
    )
